...
| Sat | 1h   | [ ]  | Skim Node “event loop” / timers docs; write **5 bullets** in your notes (“what blocks vs defers”).                           |
| Sat | 1h   | [ ]  | **Validate env** with Zod (or manual): fail fast with clear message if required vars missing.                                |
| Sat | 1h   | [ ]  | Split code: `src/config.ts` + `src/main.ts` (or `.js`); one import path clean.                                               |
| Sat | 1h   | [ ]  | **Retro (15m)** + push repo to GitHub; `.gitignore` includes `node_modules`, `.env`.                                         |
| Sun | 1h   | [ ]  | Read one stack trace end-to-end; fix a deliberate bug using only the trace.                                                  |
| Sun | 1h   | [ ]  | Add **eslint** (or biome) minimal config; fix one lint issue.                                                                |
| Sun | 1h   | [ ]  | README: prerequisites + how to run; **checkpoint**: stranger could run `npm i && npm start`.                                 |
| Sun | 1h   | [ ]  | Buffer: stretch (pre-read HTTP week) or catch-up.                                                                            |
...
| Sat | 1h   | [ ]  | Design **one resource** (e.g. `Note`): fields, routes, who can call.                                                  |
| Sat | 1h   | [ ]  | Draw request/response for **create** + **update** + **list** (paper or Excalidraw).                                   |
| Sat | 1h   | [ ]  | List **domain errors** vs **validation errors**; map each to HTTP status.                                             |
| Sat | 1h   | [ ]  | Optional: read MDN “HTTP overview” skim; bookmark for later.                                                          |
| Sun | 1h   | [ ]  | **Udemy:** Finish **Introduction + Getting Started** if not done (Nest CLI, first app in `udemy-zero-to-hero`).       |
| Sun | 1h   | [ ]  | Write **OpenAPI-style** summary (even if not YAML yet): paths + main schemas.                                         |
| Sun | 1h   | [ ]  | **Retro** + prep Week 3 (`portfolio-api` vs Udemy repo — see [two-repo rule](#udemy-nestjs-zero-to-hero-integrated)). |
| Sun | 1h   | [ ]  | Rest or catch-up; ensure Week 1 repo still runs.                                                                      |
...
| Sat | 1h   | [ ]  | **Udemy:** Any remaining Task CRUD / structure lectures.                                                                                |
| Sat | 1h   | [ ]  | **Portfolio:** Exception filter or Nest defaults; 404/400 bodies match Week 2 error shape.                                              |
| Sat | 1h   | [ ]  | **Portfolio:** In-memory store in service; **query** validation (e.g. pagination DTO) on list.                                          |
| Sat | 1h   | [ ]  | **Portfolio:** **curl** or REST Client; `docs/requests.http`.                                                                           |
| Sun | 1h   | [ ]  | **Portfolio:** Thin controller, fat service.                                                                                            |
| Sun | 1h   | [ ]  | **Portfolio:** Invalid body → 400; wrong id → 404.                                                                                      |
| Sun | 2h   | [ ]  | Buffer or catch-up; tag commit `week-3-crud`.                                                                                           |
...
| Sat | 1h   | [ ]  | **Udemy:** Finish validation/error sections you skipped.                                                                      |
| Sat | 1h   | [ ]  | **Portfolio:** Mock repository in service test OR test pure mapping logic without DB.                                         |
| Sat | 1h   | [ ]  | **Portfolio:** **Green** unit test that fails if a business rule breaks.                                                      |
| Sat | 1h   | [ ]  | **Portfolio:** README — Swagger link; **Month 1 checkpoint** vs [02-nestjs-stack-checklist.md](02-nestjs-stack-checklist.md). |
| Sun | 2h   | [ ]  | **Portfolio:** Harden duplicate create → 409 if modeled; else document “not yet”.                                             |
| Sun | 1h   | [ ]  | Retro + screenshot Swagger.                                                                                                   |
| Sun | 1h   | [ ]  | Plan Month 2 + **Udemy** logging/config preview (Week 5).                                                                     |
...
| Sat | 1h   | [ ]  | **Udemy:** Logging & configuration sections — mirror patterns in **`portfolio-api`** (`ConfigModule`, `Logger`).             |
| Sat | 1h   | [ ]  | **Portfolio:** Optional `x-request-id` → logs (interceptor or middleware).                                                   |
| Sat | 1h   | [ ]  | **Portfolio:** Replace `console.log` in hot paths with Nest `Logger`.                                                        |
| Sat | 1h   | [ ]  | **Portfolio:** Stable error JSON (**code** + **message** + traceId if added); trigger 500 in dev—no stack leak in prod mode. |
| Sun | 1h   | [ ]  | **Udemy:** Finish any logging/config lectures not done Sat.                                                                  |
| Sun | 1h   | [ ]  | **Portfolio:** Refactor `notes` to use config for port/CORS placeholder.                                                     |
| Sun | 1h   | [ ]  | Retro.                                                                                                                       |
| Sun | 1h   | [ ]  | Read Nest “Guards” doc + **Udemy** next section preview (auth).                                                              |
...
| Sat | 1h   | [ ]  | **Udemy:** TypeORM migrations / relations if still on DB section.                                                                                  |
| Sat | 1h   | [ ]  | **Portfolio:** `@nestjs/jwt` sign access token; **secret** + **expiry** from env.                                                                  |
| Sat | 1h   | [ ]  | **Portfolio:** No token logging; redact passwords in debug.                                                                                        |
| Sat | 1h   | [ ]  | **Portfolio:** Swagger auth section; unit test hash/verify path if time.                                                                           |
| Sun | 1h   | [ ]  | **Udemy:** Close out Postgres/TypeORM module in Udemy repo.                                                                                        |
| Sun | 1h   | [ ]  | **Portfolio:** **Refresh token** design on paper—stub or defer to Week 7.                                                                          |
| Sun | 1h   | [ ]  | Retro.                                                                                                                                             |
| Sun | 1h   | [ ]  | **Portfolio:** Security note in README.                                                                                                            |
...
| Sat | 1h   | [ ]  | **Udemy (≤45 min):** Guards / protected routes recap if needed.                                                  |
| Sat | 1h   | [ ]  | **Portfolio:** **Refresh** endpoint or document “phase 2”.                                                       |
| Sat | 1h   | [ ]  | **Portfolio:** Swagger **Bearer** auth; authorize in UI.                                                         |
| Sat | 1h   | [ ]  | **Portfolio:** Error matrix 401/403 + short **demo script** outline for later.                                   |
| Sun | 1h   | [ ]  | **Udemy:** Finish remaining auth/guards lectures.                                                                |
| Sun | 1h   | [ ]  | **Portfolio:** Edge cases — expired token; malformed header.                                                     |
| Sun | 1h   | [ ]  | Retro.                                                                                                           |
| Sun | 1h   | [ ]  | Prep Supertest + **Udemy** testing section for Week 8.                                                           |
...
| Sat | 1h   | [ ]  | **Udemy:** Finish **Testing** lectures if any remain.                                                                   |
| Sat | 1h   | [ ]  | **Portfolio:** Decide **test DB strategy** (mock repo vs docker postgres for tests).                                    |
| Sat | 2h   | [ ]  | **Portfolio:** Stabilize tests; reset state between tests.                                                              |
| Sat | 1h   | [ ]  | **Portfolio:** `npm run test:e2e` script ready for CI later.                                                            |
| Sun | 1h   | [ ]  | **Udemy:** Complete any remaining **Testing** + optional **Deployment** intro (watch only; deploy **portfolio** later). |
| Sun | 1h   | [ ]  | **Portfolio:** **Explain aloud** middleware vs guard vs interceptor; comment gaps.                                      |
| Sun | 1h   | [ ]  | Month 2 checkpoint vs [02-nestjs-stack-checklist.md](02-nestjs-stack-checklist.md).                                     |
| Sun | 1h   | [ ]  | Plan Prisma + Docker Week 9 — **no new TypeORM in `portfolio-api`**.                                                    |
...
| Sat | 1h   | [ ]  | Replace **Note** CRUD with Prisma.                                                                 |
| Sat | 1h   | [ ]  | Ensure **FK** or relation User → Notes if applicable.                                              |
| Sat | 1h   | [ ]  | README: Docker section “start db” + “run migrations”.                                              |
| Sat | 1h   | [ ]  | `prisma studio` quick sanity check.                                                                |
| Sun | 2h   | [ ]  | Fix integration tests to use DB (or dockerized test DB)—may spill to Week 10.                      |
| Sun | 1h   | [ ]  | Retro.                                                                                             |
| Sun | 1h   | [ ]  | Backup: export schema diagram screenshot.                                                          |
...
| Sat | 1h   | [ ]  | Query plan awareness: `EXPLAIN` one slow query on paper (optional install). |
| Sat | 2h   | [ ]  | Refactor services for **clear** repository boundaries.                      |
| Sat | 1h   | [ ]  | Integration tests updated for relations.                                    |
| Sun | 2h   | [ ]  | Buffer / test stabilization.                                                |
| Sun | 1h   | [ ]  | ERD in repo (`docs/erd.png` or Mermaid).                                    |
| Sun | 1h   | [ ]  | Retro.                                                                      |
...
| Sat | 1h   | [ ]  | Idempotency: seed safe to re-run (upsert) or document wipe.                           |
| Sat | 2h   | [ ]  | Add **integration test** for transactional flow.                                      |
| Sat | 1h   | [ ]  | Review Prisma “transactions” docs for interview talking points.                       |
| Sun | 2h   | [ ]  | Month 3 halfway checkpoint; fix failing tests.                                        |
| Sun | 1h   | [ ]  | Retro.                                                                                |
| Sun | 1h   | [ ]  | Plan Week 12 refactor scope.                                                          |
...
| Sat | 2h   | [ ]  | Harden tests; parallel vs serial decision documented.                      |
| Sat | 1h   | [ ]  | **Project 1** naming: repo rename or new folder; decision logged.          |
| Sat | 1h   | [ ]  | Checkpoint vs checklist DB + Prisma sections.                              |
| Sun | 2h   | [ ]  | Buffer / technical debt list (max 5 items).                                |
| Sun | 1h   | [ ]  | Retro Month 3.                                                             |
| Sun | 1h   | [ ]  | Skim [03-portfolio-projects.md](03-portfolio-projects.md) Project A scope. |
...
| Sat | 2h   | [ ]  | Implement **create org + add self as OWNER** API.                         |
| Sat | 1h   | [ ]  | Swagger updated.                                                          |
| Sat | 1h   | [ ]  | Seed updated for multi-tenant demo.                                       |
| Sun | 2h   | [ ]  | **Permission** checks on one resource (org-scoped).                       |
| Sun | 1h   | [ ]  | Retro.                                                                    |
| Sun | 1h   | [ ]  | Plan React app structure Week 15.                                         |
...
| Sat | 2h   | [ ]  | End-to-end **Postman/HTTP** collection for happy paths.               |
| Sat | 1h   | [ ]  | Error handling for cross-org access → 403.                            |
| Sat | 1h   | [ ]  | Integration tests: critical path (org + create entity).               |
| Sun | 2h   | [ ]  | Buffer: more tests or cut scope list.                                 |
| Sun | 1h   | [ ]  | Retro.                                                                |
| Sun | 1h   | [ ]  | Init React app if not started (Vite + TS).                            |
...
| Sat | 2h   | [ ]  | Polish UX minimally: consistent layout, form validation.                                |
| Sat | 1h   | [ ]  | CORS: local dev ports aligned; credentials if cookies.                                  |
| Sat | 1h   | [ ]  | Screenshot for future README.                                                           |
| Sun | 2h   | [ ]  | Fix cross-origin issues; document in README troubleshooting.                            |
| Sun | 1h   | [ ]  | Retro.                                                                                  |
| Sun | 1h   | [ ]  | Plan hardening Week 16.                                                                 |
...
| Sat | 2h   | [ ]  | Run through README on **fresh clone** (or ask friend); fix gaps.                               |
| Sat | 1h   | [ ]  | **Security** section: hashing, JWT, CORS, rate limit—honest limits.                            |
| Sat | 1h   | [ ]  | Tag release `v0.1-mvp`.                                                                        |
| Sun | 2h   | [ ]  | Month 4 retro; list **deploy** blockers for Month 5.                                           |
| Sun | 1h   | [ ]  | Update [03-portfolio-projects.md](03-portfolio-projects.md) checklist mentally—what’s missing. |
| Sun | 1h   | [ ]  | Rest.                                                                                          |
...
| Sat | 2h   | [ ]  | Fix flaky CI; deterministic tests.                                              |
| Sat | 1h   | [ ]  | Document “how to debug CI” in CONTRIBUTING or README.                           |
| Sat | 1h   | [ ]  | Retro.                                                                          |
| Sun | 2h   | [ ]  | Buffer / improve coverage on critical path only.                                |
| Sun | 1h   | [ ]  | Choose deploy target (Railway / Render / Fly) and read “Postgres” guide.        |
| Sun | 1h   | [ ]  | List env vars needed in prod.                                                   |
...
| Sat | 1h   | [ ]  | CORS + cookie domain for prod front URL.                                           |
| Sat | 1h   | [ ]  | HTTPS only; HSTS awareness (platform may handle).                                  |
| Sat | 1h   | [ ]  | Document rollback strategy one paragraph.                                          |
| Sat | 1h   | [ ]  | Celebrate + note URL in README.                                                    |
| Sun | 2h   | [ ]  | Fix prod-only bug if any (logging first).                                          |
| Sun | 1h   | [ ]  | Retro.                                                                             |
| Sun | 1h   | [ ]  | Plan Week 19 observability.                                                        |
...
| Sat | 2h   | [ ]  | **Error monitoring** optional (Sentry free tier) one integration—or skip with rationale in README. |
| Sat | 1h   | [ ]  | Audit **secrets**: rotate JWT secret if exposed during debug.                                      |
| Sat | 1h   | [ ]  | Retro.                                                                                             |
| Sun | 2h   | [ ]  | Load test **optional** (k6 or `autocannon`) on `/health`—document baseline.                        |
| Sun | 1h   | [ ]  | Month 5 checkpoint.                                                                                |
| Sun | 1h   | [ ]  | Decide Project 2 vs differentiator per [03-portfolio-projects.md](03-portfolio-projects.md).       |
...
| Sat | 3h   | [ ]  | Build **vertical slice** visible in UI or documented API.                       |
| Sat | 1h   | [ ]  | README section for new feature.                                                 |
| Sun | 2h   | [ ]  | Polish + edge cases.                                                            |
| Sun | 1h   | [ ]  | Retro Month 5.                                                                  |
| Sun | 1h   | [ ]  | Plan documentation Month 6.                                                     |
...
| Sat | 2h   | [ ]  | Record **2–3 min** demo (optional) or detailed GIF checklist.               |
| Sat | 1h   | [ ]  | Unify tone across both project READMEs.                                     |
| Sat | 1h   | [ ]  | Retro.                                                                      |
| Sun | 2h   | [ ]  | Peer review: ask one person to follow README; capture friction.             |
| Sun | 1h   | [ ]  | Fix top 3 README issues.                                                    |
| Sun | 1h   | [ ]  | Rest.                                                                       |
...
| Sat | 2h   | [ ]  | Resume pass: numbers, links, no buzzword soup.                                     |
| Sat | 1h   | [ ]  | Cover letter template paragraph (company research placeholder).                    |
| Sat | 1h   | [ ]  | Retro.                                                                             |
| Sun | 2h   | [ ]  | Portfolio site optional one-pager linking demos.                                   |
| Sun | 1h   | [ ]  | Ask feedback from peer or mentor.                                                  |
| Sun | 1h   | [ ]  | Rest.                                                                              |
...
| Sat | 2h   | [ ]  | Deep dive weak area from mock (e.g. indexes, websocket scale).             |
| Sat | 1h   | [ ]  | Flashcards or note sheet: 20 facts about your own codebase.                |
| Sat | 1h   | [ ]  | Retro.                                                                     |
| Sun | 2h   | [ ]  | Practice **two** stories: conflict/debug and technical decision.           |
| Sun | 1h   | [ ]  | Sleep hygiene + schedule applications for Week 24.                         |
| Sun | 1h   | [ ]  | Rest.                                                                      |
...
| Sat | 2h   | [ ]  | Portfolio tweak from interview feedback (small, high ROI only).                |
| Sat | 1h   | [ ]  | **Retro** full 6 months: what you’d tell yourself on day 1.                    |
| Sat | 1h   | [ ]  | Plan next 90 days skill goals.                                                 |
| Sun | 2h   | [ ]  | Open source **good first issue** OR rest / recovery.                           |
| Sun | 1h   | [ ]  | Celebrate milestones completed.                                                |
| Sun | 1h   | [ ]  | Optional: schedule recurring **1h/week** maintenance for pinned repos.         |
